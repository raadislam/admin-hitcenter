...
export default function StatusBadgeEditable({
  ...
  async function changeStatus(s: string) {
    ...
    try {
      ...
      onChange(s);
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 1800); // show for 1.8s
    } catch {
      setShowError(true);
  ...
  return (
    <div className="relative inline-block">
      ...
      {open && (
        <div className="absolute z-40 left-1/2 -translate-x-1/2 top-full mt-2 w-[170px] p-3 rounded-xl border bg-white shadow-lg flex flex-col gap-2">
          {statuses.map((s) => (
            <button
              key={s.value}
